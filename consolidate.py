...
#     ckpt_prefix=os.path.join("llava-v1.5-llama2-7b-finetune-5565k-bs512", "model_ckpt"), ckpt_suffix="_rank-*-of-*.pth", save_model=False)

# torch.save(state_dict, "llava-v1.5-llama2-7b-finetune-5565k-bs512/full_model.pth")

# pretrained projectors

# state_dict, _ = consolidate_sharded_model_checkpoints(
#           ckpt_prefix=os.path.join("llava-v1.5-7b-s2-sampler-convnext-dinov2-CLIP576-10samplers16-pos-pretrain", "mm_projector"), ckpt_suffix="_rank-*-of-*.pth", save_model=False)

# torch.save(state_dict, "llava-v1.5-7b-s2-sampler-convnext-dinov2-CLIP576-10samplers16-pos-pretrain/mm_projector.bin")


def main(ckpt_path, ckpt_prefix, ckpt_suffix, save_path):
    ...
    if not os.path.exists(ckpt_path):
        ...
        print(f"Save path {save_path} already exists. Overwriting...")
        return
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(args.ckpt_path, args.state_dict_name)

    main(args.ckpt_path, args.ckpt_prefix, args.ckpt_suffix, save_path)
